...
const CASH_KEY = "@kasirapp_cash_manual";
...
export default function HomeScreen({ user, onLogout, onUpdateUser }: Props) {
  ...
  const cashManualHariIni = cashManual.filter(
    (c) => c.tanggal === selectedDateStr,
  );
  const cashManualMasuk = cashManualHariIni
    .filter((c) => c.tipe === "masuk")
    .reduce((s, c) => s + c.jumlah, 0);
  const cashManualKeluar = cashManualHariIni
  ...
  const allCashManualIn = cashManual
    .filter((c) => c.tipe === "masuk")
    .reduce((s, c) => s + c.jumlah, 0);
  const allCashManualOut = cashManual
    .filter((c) => c.tipe === "keluar")
    .reduce((s, c) => s + c.jumlah, 0);
  ...
  const allCashHariIni: CashRecord[] = [
    ...txHariIni.map((t) => ({
      ...
      tipe: "masuk" as const,
      jumlah: t.total,
      keterangan: `Penjualan (${t.items?.length ?? 0} item)`,
      noNota: t.noNota,
    })),
    ...cashManualHariIni,
  ...
  const handleAbsenKeluar = async () => {
    ...
    const updated = absensi.map((a) =>
      a.tanggal === todayString() && !a.keluar ? { ...a, keluar: time } : a,
    );
    setAbsensi(updated);
    setShowAbsenModal(false);
    await AsyncStorage.setItem(ABSENSI_KEY, JSON.stringify(updated));
  ...
  const handleAbsenCustom = async (status: "cuti" | "izin") => {
    ...
    Alert.alert(
      "✅",
      `${status === "cuti" ? "Cuti" : "Izin"} berhasil dicatat`,
    );
  };
  const handleAddCash = async () => {
    if (!cashJumlah.trim() || isNaN(Number(cashJumlah))) {
      Alert.alert("Perhatian", "Masukkan jumlah yang valid");
      return;
    }
    if (!cashKet.trim()) {
      Alert.alert("Perhatian", "Keterangan tidak boleh kosong");
      return;
    }
    const r: CashRecord = {
      id: genId(),
      ...
      keterangan: cashKet.trim(),
    };
    const updated = [r, ...cashManual];
    setCashManual(updated);
    setCashJumlah("");
    setCashKet("");
    ...
    Alert.alert(
      "✅ Berhasil",
      `Kas ${cashTipe === "masuk" ? "masuk" : "keluar"} berhasil dicatat`,
    );
  };
  ...
  const DateNavigator = () => (
    <View style={styles.dateNav}>
      <TouchableOpacity
        ...
        onPress={() => setSelectedDate((d) => prevDay(d))}
      >
        <Ionicons name="chevron-back" size={18} color={C.primaryLight} />
      </TouchableOpacity>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          if (!isToday(selectedDate)) setSelectedDate((d) => nextDay(d));
        }}
        disabled={isToday(selectedDate)}
      >
        <Ionicons name="chevron-forward" size={18} color={C.primaryLight} />
      </TouchableOpacity>
    </View>
  );
    ...
    <View style={styles.root}>
      ...
      <Animated.View style={[styles.header, { opacity: headerFade }]}>
        ...
        <View
          style={[
            styles.hCircle,
            { width: 120, height: 120, bottom: -30, left: -30, opacity: 0.06 },
          ]}
        />
        ...
        <View style={styles.kasCard}>
          <View style={styles.kasTop}>
            <View>
              <Text style={styles.kasLbl}>
                Saldo Kas{" "}
                {isToday(selectedDate)
                  ? "Hari Ini"
                  : selectedDateStr.split(" ").slice(0, 2).join(" ")}
              </Text>
          ...
          <View style={styles.kasRow}>
            {[
              { lbl: "Penjualan", val: kasMasukDariTx, dot: C.accent },
              ...
              <React.Fragment key={i}>
                ...
                <View style={styles.kasItem}>
                  <View
                    style={[styles.kasDot, { backgroundColor: item.dot }]}
                  />
      ...
      >
        ...
        >
          {sudahAbsen ? (
            <View style={styles.absenRow}>
              <View
                style={[styles.absenIcon, { backgroundColor: C.primaryBg }]}
              >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.absenSub}>
                  {absensi.find((a) => a.tanggal === todayString())?.masuk} —
                  Semangat kerja!
                </Text>
              </View>
              <TouchableOpacity
                style={styles.absenKelBtn}
                onPress={() => setShowAbsenModal(true)}
              >
        ...
        <Animated.View
          style={[styles.quickGrid, { transform: [{ translateY: cardSlide }] }]}
        >
          {[
            {
              icon: "cart",
        ...
        {txHariIni.length === 0 ? (
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptyTxt}>
              Belum ada transaksi{" "}
              {isToday(selectedDate) ? "hari ini" : "di tanggal ini"}
            </Text>
            {isToday(selectedDate) && (
              <TouchableOpacity
          ...
          <View style={styles.listCard}>
            <View style={styles.txSummaryRow}>
              <View style={styles.txSumItem}>
                <Text style={[styles.txSumVal, { color: C.primary }]}>
                  {txHariIni.length}
                </Text>
              ...
              <View style={styles.txSumItem}>
                <Text
                  style={[styles.txSumVal, { color: C.accent, fontSize: 14 }]}
                  numberOfLines={1}
                >
              ...
              <View style={styles.txSumItem}>
                <Text
                  style={[styles.txSumVal, { color: "#f97316", fontSize: 14 }]}
                  numberOfLines={1}
                >
                  {txHariIni.reduce((s, t) => s + (t.items?.length ?? 0), 0)}
                </Text>
            ...
            {txHariIni.slice(0, 4).map((tx, i) => (
              <View
                ...
                style={[
                  styles.txItem,
                  i === Math.min(3, txHariIni.length - 1) && {
                    borderBottomWidth: 0,
                  },
                ]}
              >
                ...
                <View style={{ flex: 1, marginLeft: 12 }}>
                  ...
                  <Text style={styles.txMeta}>
                    {tx.jam} · {tx.items?.length ?? 0} item · {tx.kasir}
                  </Text>
                </View>
          ...
          <View style={styles.listCard}>
            {allCashHariIni.slice(0, 5).map((item, i) => (
              <View
                ...
                style={[
                  styles.cashItem,
                  i === Math.min(4, allCashHariIni.length - 1) && {
                    borderBottomWidth: 0,
                  },
                ]}
              >
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        item.tipe === "masuk" ? C.primaryBg : "#fef2f2",
                    },
                  ]}
                >
                  <Ionicons
                    ...
                    size={20}
                    color={item.tipe === "masuk" ? C.accent : "#ef4444"}
                  />
                </View>
                <View style={{ flex: 1, marginLeft: 12 }}>
        ...
        <View style={styles.listCard}>
          <View style={styles.absenStats}>
            {[
              {
                lbl: "Hadir",
                val: absensi.filter((a) => a.status === "hadir").length,
                c: C.accent,
              },
              {
                lbl: "Cuti",
                val: absensi.filter((a) => a.status === "cuti").length,
                c: "#f59e0b",
              },
              {
                lbl: "Izin",
                val: absensi.filter((a) => a.status === "izin").length,
                c: C.primary,
              },
              ...
              <View key={i} style={{ flex: 1, alignItems: "center" }}>
                <Text
                  style={[{ fontSize: 20, fontWeight: "800" }, { color: s.c }]}
                >
                  {s.val}
                </Text>
          ...
          {absensi.length === 0 ? (
            ...
            absensi.slice(0, 3).map((item, i) => (
              <View
                key={item.id}
                style={[styles.absenItem, i === 2 && { borderBottomWidth: 0 }]}
              >
                <View
                  style={[
                    styles.absenDot,
      ...
      {/* ══ MODAL: ABSENSI ══ */}
      <Modal
        visible={showAbsenModal}
        transparent
        animationType="slide"
        onRequestClose={() => setShowAbsenModal(false)}
      >
        <View style={styles.overlay}>
          <TouchableOpacity
            style={{ flex: 1 }}
            onPress={() => setShowAbsenModal(false)}
          />
            ...
            {!sudahAbsen ? (
              <>
                ...
                <View style={{ flexDirection: "row", gap: 12, marginTop: 12 }}>
                  {[
                    { lbl: "Catat Cuti", c: "#f59e0b", s: "cuti" as const },
                    { lbl: "Catat Izin", c: C.primary, s: "izin" as const },
                  ].map((opt, i) => (
                    <TouchableOpacity
                      key={i}
                      style={[styles.optBtn, { borderColor: opt.c + "40" }]}
                      onPress={() => handleAbsenCustom(opt.s)}
                    >
                      <Text
                        style={[
        ...
        <View style={styles.overlay}>
          ...
          <View style={styles.sheet}>
              ...
              {(["masuk", "keluar"] as const).map((t) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.toggle,
                    cashTipe === t && {
                      backgroundColor: t === "masuk" ? C.accent : "#ef4444",
                      borderColor: "transparent",
                    },
                  ]}
                  onPress={() => setCashTipe(t)}
                >
                  <Ionicons
                    name={
                      t === "masuk"
                    ...
                    size={16}
                    color={cashTipe === t ? "#fff" : "#64748b"}
                  />
                  <Text
                    style={[
                      { fontSize: 14, fontWeight: "700", color: "#64748b" },
                      cashTipe === t && { color: "#fff" },
                    ]}
                  >
                    Kas {t === "masuk" ? "Masuk" : "Keluar"}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View
              style={[
                styles.cashInput,
                focusCash === "j" && { borderColor: C.primary },
              ]}
            >
              <Text
                style={{
              ...
              <TextInput
                ...
                value={cashJumlah}
                onChangeText={(v) => setCashJumlah(v.replace(/[^0-9]/g, ""))}
                keyboardType="numeric"
                onFocus={() => setFocusCash("j")}
                onBlur={() => setFocusCash(null)}
              />
            </View>
            ...
            <View
              style={[
                ...
                focusCash === "k" && { borderColor: C.primary },
              ]}
            >
              <TextInput
                style={{ fontSize: 15, color: "#0f172a", paddingVertical: 13 }}
                placeholder="Contoh: Beli bahan baku..."
                placeholderTextColor="#94a3b8"
                value={cashKet}
                onChangeText={setCashKet}
                onFocus={() => setFocusCash("k")}
                onBlur={() => setFocusCash(null)}
              />
            </View>
            <View style={{ flexDirection: "row", gap: 12 }}>
              ...
              <TouchableOpacity
                style={[
                  ...
                  {
                    backgroundColor:
                      cashTipe === "masuk" ? C.accent : "#ef4444",
                  },
                ]}
      ...
      <Modal
        visible={showTxList}
        animationType="slide"
        onRequestClose={() => setShowTxList(false)}
      >
        <View style={styles.fullModal}>
          <View style={styles.fullHeader}>
            <TouchableOpacity
              style={styles.fullBackBtn}
              onPress={() => setShowTxList(false)}
            >
          ...
          <View style={styles.summaryBar}>
            <View style={styles.sumItem}>
              <Text style={[styles.sumVal, { color: C.primary }]}>
                {transactions.length}
              </Text>
            ...
            <View style={styles.sumItem}>
              <Text
                style={[styles.sumVal, { color: C.accent, fontSize: 13 }]}
                numberOfLines={1}
              >
                {formatRp(totalTxAllTime)}
              </Text>
              <Text style={styles.sumLbl}>Pendapatan</Text>
            </View>
            <View style={styles.sumDiv} />
            <View style={styles.sumItem}>
              <Text style={[styles.sumVal, { color: "#f97316" }]}>
                {txHariIni.length}
              </Text>
              <Text style={styles.sumLbl}>Hari Ini</Text>
            </View>
          </View>
          <FlatList
            data={transactions}
            ...
            ListEmptyComponent={
              <View style={{ alignItems: "center", paddingTop: 60 }}>
                ...
                <Text style={{ fontSize: 15, color: "#94a3b8", marginTop: 12 }}>
                  Belum ada transaksi
                </Text>
              </View>
            }
            renderItem={({ item }) => (
              <View style={styles.txListCard}>
                <View
                  style={[styles.txListIcon, { backgroundColor: C.primaryBg }]}
                >
                  <Ionicons name="receipt" size={20} color={C.primary} />
                </View>
                <View style={{ flex: 1, marginLeft: 12 }}>
                  <Text style={styles.txListNota}>{item.noNota}</Text>
                  <Text style={styles.txListMeta}>
                    {item.tanggal} · {item.jam}
                  </Text>
                  <Text style={styles.txListMeta}>
                    {item.items?.length ?? 0} item · {item.kasir}
                  </Text>
                </View>
                <View style={{ alignItems: "flex-end" }}>
                  <Text style={[styles.txListTotal, { color: C.primary }]}>
                    {formatRp(item.total)}
                  </Text>
                  <View style={styles.lnsBadge}>
                    <Text style={styles.lnsTxt}>Lunas</Text>
                  </View>
                </View>
              </View>
            )}
      ...
      {/* ══ MODAL: DAFTAR KAS ══ */}
      <Modal
        visible={showCashList}
        animationType="slide"
        onRequestClose={() => setShowCashList(false)}
      >
        <View style={styles.fullModal}>
          <View style={styles.fullHeader}>
            <TouchableOpacity
              style={styles.fullBackBtn}
              onPress={() => setShowCashList(false)}
            >
              <Ionicons name="arrow-back" size={22} color="#fff" />
            </TouchableOpacity>
            <View style={{ flex: 1, alignItems: "center" }}>
            ...
            <TouchableOpacity
              style={[styles.fullBackBtn, { backgroundColor: C.accent }]}
              onPress={() => {
                setShowCashList(false);
          ...
          <View style={styles.summaryBar}>
            <View style={styles.sumItem}>
              <Text
                ...
                numberOfLines={1}
              >
                {formatRp(totalTxAllTime + allCashManualIn)}
              </Text>
              <Text style={styles.sumLbl}>Total Masuk</Text>
            </View>
            <View style={styles.sumDiv} />
            <View style={styles.sumItem}>
              <Text
                style={[styles.sumVal, { color: "#ef4444", fontSize: 13 }]}
                numberOfLines={1}
              >
                {formatRp(allCashManualOut)}
              </Text>
              <Text style={styles.sumLbl}>Total Keluar</Text>
            </View>
            <View style={styles.sumDiv} />
            <View style={styles.sumItem}>
              <Text
                style={[styles.sumVal, { color: C.primary, fontSize: 13 }]}
                numberOfLines={1}
              >
                {formatRp(saldoTotal)}
              </Text>
      ...
      <Modal
        visible={showAbsenList}
        animationType="slide"
        onRequestClose={() => setShowAbsenList(false)}
      >
        <View style={styles.fullModal}>
          <View style={styles.fullHeader}>
            <TouchableOpacity
              style={styles.fullBackBtn}
              onPress={() => setShowAbsenList(false)}
            >
              <Ionicons name="arrow-back" size={22} color="#fff" />
            </TouchableOpacity>
            <View style={{ flex: 1, alignItems: "center" }}>
          ...
          <View style={styles.summaryBar}>
            {[
              ...
              <React.Fragment key={i}>
                <View style={styles.sumItem}>
                  <Text style={[styles.sumVal, { color: s.c }]}>{s.val}</Text>
...
function BottomNav({
  ...
  const tabs = [
    ...
    },
    {
      ...
      iconOut: "receipt-outline" as const,
      lbl: "Kasir",
    },
    {
      id: "produk" as const,
      icon: "cube" as const,
      iconOut: "cube-outline" as const,
      lbl: "Produk",
    },
    {
      id: "profil" as const,
      icon: "person-circle" as const,
      iconOut: "person-circle-outline" as const,
      lbl: "Profil",
    },
  ];
  return (
    <View style={styles.bottomNav}>
      {tabs.map((t) => (
        ...
        >
          ...
          >
            <Ionicons
              name={activeTab === t.id ? t.icon : t.iconOut}
              size={activeTab === t.id ? 22 : 24}
              color={activeTab === t.id ? C.primary : "#94a3b8"}
            />
          </View>
          <Text
            style={[
              styles.navLbl,
              activeTab === t.id && { color: C.primary, fontWeight: "700" },
            ]}
          >
            {t.lbl}
          </Text>
...
const styles = StyleSheet.create({
  ...
  greet: { fontSize: 13, color: C.primaryLight, marginBottom: 2 },
  ...
  tokoChipTxt: { fontSize: 12, color: C.primaryLight },
  bellBtn: {
    width: 40,
    height: 40,
    borderRadius: 12,
  ...
  kasLbl: { fontSize: 12, color: C.primaryLight, marginBottom: 4 },
  kasAmt: { fontSize: 24, fontWeight: "900", color: "#fff" },
  kasSub: { fontSize: 11, color: C.primaryLight, marginTop: 2 },
  kasAddBtn: {
    flexDirection: "row",
    alignItems: "center",
    gap: 5,
  ...
  kasItemLbl: { fontSize: 10, color: C.primaryLight, marginBottom: 2 },
  ...
  secLink: { fontSize: 13, fontWeight: "600", color: C.primary },
  quickGrid: {
    flexDirection: "row",
    flexWrap: "wrap",
    paddingHorizontal: 12,
  ...
  txTotal: { fontSize: 14, fontWeight: "800", color: C.primary },
  cashItem: {
    flexDirection: "row",
    alignItems: "center",
    paddingHorizontal: 16,
  ...
  navIconActive: { backgroundColor: C.primaryBg },
  ...
  fullSub: { fontSize: 12, color: C.primaryLight, marginTop: 2 },
  summaryBar: {
    flexDirection: "row",
    backgroundColor: "#fff",
    margin: 16,
  ...
  txListTotal: { fontSize: 15, fontWeight: "800" },
  lnsBadge: {
    backgroundColor: C.primaryBg,
    borderRadius: 8,
    paddingHorizontal: 8,
  ...
  lnsTxt: { fontSize: 11, fontWeight: "700", color: C.primaryMid },
});
