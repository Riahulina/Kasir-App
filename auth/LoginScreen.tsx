...
import { loginUser, UserProfile } from "../auth/AuthService";
...
const C = {
  ...
  primaryDark: "#14532d", // hijau gelap (header)
  primaryLight: "#bbf7d0", // hijau muda
  accent: "#22c55e", // hijau terang
  indigo: "#16a34a", // alias untuk focus border
};
...
export default function LoginScreen({
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      onLoggedIn(profile); // kirim profile ke App.tsx
    } catch (e: any) {
      const code = e?.code ?? "";
      if (
        [
      ...
      } else if (code === "auth/profile-not-found") {
        setError("Profil tidak ditemukan, hubungi admin");
      } else {
        setError("Login gagal: " + (e?.message ?? "coba lagi"));
      }
      shake();
    } finally {
      setLoading(false);
    ...
    <View style={s.root}>
      ...
      <View style={s.header}>
        <View
          style={[
            s.circle,
            { width: 280, height: 280, top: -100, right: -80, opacity: 0.12 },
          ]}
        />
        <View
          style={[
            s.circle,
            { width: 160, height: 160, top: 30, left: -50, opacity: 0.08 },
          ]}
        />
        <Animated.View
          style={[
      ...
      >
        ...
        >
          ...
          <Text style={s.footerNote}>
            ...
            <Text style={{ color: C.primary }}>Syarat & Ketentuan</Text> dan{" "}
            <Text style={{ color: C.primary }}>Kebijakan Privasi</Text>
          </Text>
        </Animated.View>
      </ScrollView>
    </View>
...
const s = StyleSheet.create({
  ...
  brandTagline: { fontSize: 14, color: "#bbf7d0", marginTop: 4 },
  card: {
    backgroundColor: "#fff",
  ...
  forgotTxt: { fontSize: 13, color: "#16a34a", fontWeight: "600" },
  inputBox: {
    flexDirection: "row",
    alignItems: "center",
  ...
  btnOutlineTxt: { fontSize: 15, fontWeight: "700", color: "#16a34a" },
  footerNote: {
    fontSize: 11.5,
    color: "#94a3b8",
    textAlign: "center",
