...
export default function ProfileScreen({ user, onLogout, onUpdateUser }: Props) {
  ...
  const saveProfile = async (p: UserProfile) => {
    setProfile(p);
    await AsyncStorage.setItem(PROFILE_KEY, JSON.stringify(p)).catch(
      console.error,
    );
  ...
  const handleChangePhoto = () =>
    Alert.alert("Ganti Foto Profil", "Pilih sumber", [
      { text: "Galeri", onPress: pickGallery },
      { text: "Kamera", onPress: pickCamera },
      {
        text: "Hapus Foto",
        style: "destructive",
        onPress: () => saveProfile({ ...profile, photoUri: undefined }),
      },
  ...
  const pickGallery = async () => {
    ...
    if (!res.canceled && res.assets[0])
      saveProfile({ ...profile, photoUri: res.assets[0].uri });
  };

  const pickCamera = async () => {
    const { status } = await ImagePicker.requestCameraPermissionsAsync();
    if (status !== "granted") {
      Alert.alert("Izin diperlukan", "Berikan izin akses kamera");
      return;
    }
  ...
  const handleChangePassword = async () => {
    if (!fOldPass.trim()) {
      Alert.alert("Perhatian", "Masukkan password lama");
      return;
    }
    ...
    try {
      ...
      if (uJson) {
        ...
        if (users[key]?.password !== fOldPass) {
  ...
  // ── Edit toko ─────────────────────────────────────────────────────────────
  const openTokoModal = () => {
    setFNamaToko(profile.namaToko || "");
    setFAlamat(profile.alamat || "");
    setFTelepon(profile.telepon || "");
    setFKota(profile.kota || "");
    setTokoModal(true);
  };
  const handleSaveToko = () => {
    ...
    saveProfile({
      ...profile,
      namaToko: fNamaToko.trim(),
      alamat: fAlamat.trim(),
      telepon: fTelepon.trim(),
      kota: fKota.trim(),
    });
    setTokoModal(false);
  ...
  // ── Logout ────────────────────────────────────────────────────────────────
  const handleLogout = () =>
    Alert.alert("Keluar", "Yakin ingin logout dari akun ini?", [
      { text: "Batal", style: "cancel" },
    ...
    <View style={s.root}>
      ...
      >
        ...
        <View style={s.profileCard}>
          ...
          {profile.namaToko && (
            <View style={s.tokoBadge}>
              <Ionicons name="storefront-outline" size={13} color="#22c55e" />
        ...
        <View style={s.statsRow}>
          {[
            ...
            <View key={i} style={s.statCard}>
              ...
              <Text style={[s.statVal, { color: st.c }]} numberOfLines={1}>
                {st.val}
              </Text>
        ...
        <View style={s.menuGroup}>
          ...
          <MenuItem
            ...
            color="#22c55e"
            label="Foto Profil"
            sub="Ubah foto dari galeri atau kamera"
            onPress={handleChangePhoto}
            last
          />
        </View>
        ...
        <View style={s.menuGroup}>
          <MenuItem
            ...
            sub={profile.namaToko || "Belum diisi"}
            onPress={openTokoModal}
          />
          <MenuItem
        ...
        <View style={s.menuGroup}>
          ...
          <ToggleItem
            ...
            sub="Langsung cetak setelah transaksi"
            value={settings.cetakOtomatis}
            onToggle={() => toggle("cetakOtomatis")}
            toggleColor="#a855f7"
          />
          {/* Tampilkan stok */}
          <ToggleItem
            ...
            bg="#f0fdf4"
            color="#22c55e"
            label="Tampilkan Stok di Kasir"
            sub="Stok produk terlihat saat transaksi"
            value={settings.tampilkanStok}
            onToggle={() => toggle("tampilkanStok")}
            toggleColor="#22c55e"
            last
          />
        </View>
        ...
        <View style={s.menuGroup}>
          <MenuItem
            ...
            sub={`${totalTx} transaksi tersimpan`}
            onPress={() => setTxModal(true)}
            last
          />
        </View>
          ...
          <View style={s.recentCard}>
            ...
            {receipts.slice(0, 3).map((r, i) => (
              <TouchableOpacity
                ...
                style={[
                  s.txItem,
                  i === Math.min(2, receipts.length - 1) && {
                    borderBottomWidth: 0,
                  },
                ]}
                onPress={() => setDetailModal(r)}
              >
                <View style={s.txIcon}>
        ...
        <View style={s.menuGroup}>
          <ToggleItem
            ...
            color="#a855f7"
            label="Notifikasi"
            sub="Tampilkan notifikasi aplikasi"
          ...
          <ToggleItem
            ...
            color="#f97316"
            label="Suara"
            sub="Suara saat transaksi berhasil"
            value={settings.suara}
            onToggle={() => toggle("suara")}
            toggleColor="#f97316"
            last
          />
        </View>
        ...
        <View style={s.menuGroup}>
          <MenuItem
            ...
            color="#64748b"
            label="Bantuan & FAQ"
            sub="Panduan penggunaan aplikasi"
            onPress={() =>
              Alert.alert("Bantuan", "Hubungi: support@kasirapp.id")
            }
          />
          <MenuItem
            ...
            sub="KasirApp v1.0.0"
            onPress={() =>
              Alert.alert("KasirApp", "Versi 1.0.0\nPoliteknik Negeri Medan")
            }
            last
          />
        </View>
        ...
        <Text style={s.versionTxt}>
          KasirApp v1.0.0 · Politeknik Negeri Medan
        </Text>
      </ScrollView>
      ...
      >
        ...
        <SField
          ...
          secure
          showSecure={showOld}
          toggleSecure={() => setShowOld((p) => !p)}
        />
        <SField
          label="Password Baru *"
          value={fNewPass}
          onChange={setFNewPass}
          ...
          secure
          showSecure={showNew}
          toggleSecure={() => setShowNew((p) => !p)}
        />
        <SField
          label="Konfirmasi Password *"
          value={fConfPass}
          onChange={setFConfPass}
          ...
          secure
          showSecure={showConf}
          toggleSecure={() => setShowConf((p) => !p)}
        />
        <ModalActions
          onCancel={() => setPassModal(false)}
          onSave={handleChangePassword}
          saveLabel="Ubah Password"
          saveColor="#f97316"
        />
      </BottomSheet>
      ...
      >
        ...
        />
        <ModalActions
          onCancel={() => setTokoModal(false)}
          onSave={handleSaveToko}
          saveLabel="Simpan Info Toko"
          saveColor="#f97316"
        />
      </BottomSheet>
      ...
      <Modal
        visible={txModal}
        animationType="slide"
        onRequestClose={() => setTxModal(false)}
      >
        <View style={s.fullModal}>
          ...
          <View style={s.txSummaryBar}>
            <View style={s.txSumItem}>
              <Text style={[s.txSumVal, { color: "#22c55e" }]}>{totalTx}</Text>
              <Text style={s.txSumLbl}>Transaksi</Text>
            </View>
            <View style={s.txSumDiv} />
            <View style={s.txSumItem}>
              <Text
                style={[s.txSumVal, { color: "#22c55e", fontSize: 13 }]}
                numberOfLines={1}
              >
                {formatRp(totalPendapatan)}
              </Text>
              <Text style={s.txSumLbl}>Pendapatan</Text>
            </View>
          </View>
          <FlatList
            data={receipts}
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  setTxModal(false);
                  setTimeout(() => setDetailModal(item), 300);
                }}
              >
                <View style={[s.txListIcon, { backgroundColor: "#ecfeff" }]}>
                  <Ionicons name="receipt" size={20} color="#06b6d4" />
                </View>
                <View style={{ flex: 1, marginLeft: 12 }}>
                  <Text style={s.txListNota}>{item.noNota}</Text>
                  <Text style={s.txListMeta}>
                ...
                <View style={{ alignItems: "flex-end" }}>
                  ...
                  <View style={s.lunasBadge}>
                    <Text style={s.lunasTxt}>Lunas</Text>
                  </View>
                </View>
              </TouchableOpacity>
            )}
          ...
          <View style={s.detailSheet}>
            ...
            >
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                ...
                <Text style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>
                  {detailModal?.tanggal} · {detailModal?.jam}
                </Text>
              </View>
              <TouchableOpacity
                style={s.closeBtn}
            ...
            <View style={s.detailRows}>
              {[
                ...
                {
                  lbl: "Item",
                  val: `${detailModal?.items?.length ?? 0} produk`,
                },
                { lbl: "Subtotal", val: formatRp(detailModal?.total || 0) },
                { lbl: "Dibayar", val: formatRp(detailModal?.bayar || 0) },
                {
                  lbl: "Kembalian",
                  val: formatRp(detailModal?.kembalian || 0),
                  highlight: true,
                },
              ].map((row, i) => (
                <View
                  key={i}
                  style={[s.detailRow, i === 4 && { borderBottomWidth: 0 }]}
                >
                  <Text style={s.detailLbl}>{row.lbl}</Text>
                  <Text
                    style={[
                      s.detailVal,
                      row.highlight && { color: "#22c55e", fontWeight: "800" },
                    ]}
                  >
                    {row.val}
                  </Text>
            ...
            <View style={s.detailTotalBox}>
              ...
              <Text style={{ fontSize: 22, fontWeight: "900", color: "#fff" }}>
                {formatRp(detailModal?.total || 0)}
              </Text>
            </View>
            <TouchableOpacity
              style={s.closeBtnFull}
              onPress={() => setDetailModal(null)}
            >
              <Text
...
function MenuItem({
  ...
  return (
    <TouchableOpacity
      style={[s.menuItem, s.menuRow, last && s.menuLast]}
      onPress={onPress}
      activeOpacity={0.7}
    >
  ...
  return (
    <View style={[s.menuItem, s.menuRow, last && s.menuLast]}>
      ...
      <Switch
        ...
        trackColor={{ false: "#e2e8f0", true: toggleColor }}
        thumbColor="#fff"
      />
    </View>
  );
  ...
  return (
    <View style={{ marginBottom: 14 }}>
      ...
      <View
        style={[
          s.fieldInput,
          focused === fieldKey && s.fieldFocus,
          multiline && { alignItems: "flex-start", paddingTop: 4 },
        ]}
      >
        <TextInput
          style={[
...
function ModalActions({
  ...
  return (
    <View style={{ flexDirection: "row", gap: 12, marginTop: 8 }}>
      ...
      <TouchableOpacity
        style={[s.saveBtn, saveColor && { backgroundColor: saveColor }]}
        onPress={onSave}
      >
...
const s = StyleSheet.create({
  ...
  tokoBadgeTxt: { fontSize: 13, color: "#22c55e", fontWeight: "600" },
  editBtn: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
  ...
  editBtnTxt: { fontSize: 14, fontWeight: "700", color: "#22c55e" },
  statsRow: {
    flexDirection: "row",
    marginHorizontal: 16,
    marginTop: 12,
  ...
  lihatTxt: { fontSize: 13, fontWeight: "700", color: "#22c55e" },
  logoutBtn: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
  ...
  fieldFocus: { borderColor: "#22c55e", backgroundColor: "#fff" },
  fieldTxt: {
    flex: 1,
    fontSize: 15,
    color: "#1e293b",
